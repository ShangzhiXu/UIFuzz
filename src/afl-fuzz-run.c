...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  if (!afl->fsrv.fsrv_pid) {
    ...
    //in qemu mode, afl->argv is "/home/szxu/AFLplusplus/afl-qemu-trace"
  ...
  if (unlikely(afl->shm.cmplog_mode)) {// have no use for qemu?

    (void)write_to_testcase(afl, (void **)&use_mem, q->len, 1);

    fault = fuzz_run_target(afl, &afl->fsrv, use_tmout); // if child process return normally,
                                                                      // the is fault 0
    //change the shm
  ...
  start_us = get_cur_time_us();
    ...
    if (!afl->fsrv.nyx_mode) {
      ...
      memset(afl->fsrv.trace_bits, 0, afl->fsrv.map_size);//set to zero and restart
      MEM_BARRIER();
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {//get inside the loop.run one target for 3-8times
    ...
    (void)write_to_testcase(afl, (void **)&use_mem, q->len, 1);// get the input file

    fault = fuzz_run_target(afl, &afl->fsrv, use_tmout);
    //in afl->fsrv, there are out_file = 0x5555555cf440 "/home/szxu/Desktop/12_9/default/.cur_input",
    // target_path = 0x5555555cf700 "/home/szxu/AFLplusplus/afl-qemu-trace"
    //
    ...
    if (q->exec_cksum != cksum) {//if new

      hnb = has_new_bits(afl, afl->virgin_bits);
      OKF("has_new_bits: %d", hnb);
      if (hnb > new_bits) { new_bits = hnb; }
...
void sync_fuzzers(afl_state_t *afl) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    //this can be changed here, as aflsmart does not support to be a main. In UIFuzz, aflsmart should be able to extract from other one.
    ...
    for (o = m; o < n; o++) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);//here to run the target, and the bitmap is reset to 0 before start exec.
