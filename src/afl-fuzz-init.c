...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {//in the loop, here is to run all the items in the queue
    ...
    res = calibrate_case(afl, q, use_mem, 0, 1);//here to perform fuzz_run_target,run one target
