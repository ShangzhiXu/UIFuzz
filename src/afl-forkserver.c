...
/*#ifdef __linux__
  if (!fsrv->nyx_mode) {
    ...
    memset(fsrv->trace_bits, 0, fsrv->map_size);//set to zero and restart
    MEM_BARRIER();
  ...
  if ((res = write(fsrv->fsrv_ctl_fd, &write_value, 4)) != 4) {//write here
  ...
  if ((res = read(fsrv->fsrv_st_fd, &fsrv->child_pid, 4)) != 4) {//want to get the state returned
  ...
  fsrv->total_execs++;
  OKF("total_execs = %llu", fsrv->total_execs);
